...
This notebook demonstrates that many of the complex non-linear fitting procedures discussed in the lecture can be easily implemented in R. The notebook uses the same Wage dataset featured in the accompanying lecture on non-linear regression and is directly inspired by ISLR's Chapter 7 Lab: Non-linear Modeling.
...
The ultimate task is to propose a multivariate model that employs all the relevant predictors with an appropriate choice of their non-linear basis. When dealing with precomputed basis functions (bs,ns), we can simply combine them in a single lm() call and fit the model using least squares. However, smoothing splines and local regression do not provide explicit basis functions. In these cases, the gam library must be used to include them as components in a larger regression model.
...
1.  Is there any way to further improve the model tested in the last chunk using only these three predictors?

-   We could tune the degrees of freedom of the spline for age or try a smooth term for year instead of linear.
-   But we tested gam with smooth term and we found out that adding a smooth term for year didn’t improve the model:
    -   anova(gam.m1,gam.m2,gam.m3,test="F") compare the models, the linear year model works the best, m2 is preferred
-   So we are left only with tunning degree of freedom for age predictor.

2.  Have a look at other predictors. What treatment would you recommend for them?

-   year -\> continuous, treat it linearly
-   age -\> continuous, treat it as spline or polynomial
-   maritl -\> categorical, treat it as dummy variable
-   race -\> categorical, treat it as dummy variable
-   education -\> categorical, treat it as dummy variable
-   region -\> excluded
-   jobclass -\> categorical, treat it as dummy variable
-   health -\> categorical, treat it as dummy variable
-   health_ins -\> categorical, treat it as dummy variable

3.  Which non-linear model would you recommend for wage prediction considering all the predictors? Show a model that improves the gam model tested in the last chunk.

I recommend using a gam model for wage prediction that includes all available predictors. In this model, year is treated linearly, age is modeled using a spline to capture its nonlinear effect, and the remaining categorical variables are included as dummy variables.

```{r}
pred <- rep(NA,nrow(Wage))
for (f in seq(nFolds)){
  gam.full=gam(wage ~ year + s(age, 6) + education + maritl + race +
                 jobclass + health + health_ins,data=Wage[-myFolds[f][[1]],])
  pred[myFolds[f][[1]]] <- predict(gam.full,Wage[myFolds[f][[1]],])
}
myRMSE(pred,Wage$wage)
```
