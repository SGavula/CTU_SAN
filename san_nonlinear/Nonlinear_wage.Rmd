...
year: continuous but with few unique values → treat as linear or factor (likely linear).
maritl: categorical → treat as factor.
race: categorical → treat as factor.
education: categorical → factor.
jobclass: binary → treat as factor.
health: binary → treat as factor.
health_ins: binary → treat as factor.
logwage: target variable; excluded as predictor.
