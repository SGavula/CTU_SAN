...
1.  Which method is expected to perform best on the test data in this task (LDA, QDA or LR)? Answer without running any tests, using only the assumptions underlying each method. If the LDA assumptions truly hold (class-conditional normals and equal covariance matrices),

-   LDA is the Bayes-optimal linear rule — it will have the smallest classification error among linear rules under those assumptions.
-   Logistic Regression (LR) is a discriminative model that estimates the decision boundary directly; with enough data it will perform similarly to LDA when the LDA assumptions hold.
-   QDA is more flexible (allows different covariances) so it has more parameters. If the true covariances are equal (as in this dataset), QDA adds unnecessary flexibility and can overfit when the training set is small. With very large training sets QDA can match LDA/LR.
-   Conclusion: because the notebook data meets LDA assumptions, LDA (or LR) is expected to generalize best on test data for small-to-moderate training sizes; QDA may overfit when training data are small.
