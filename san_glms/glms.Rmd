...
poiss_reg3 <- glm(Total.Number.of.Family.members ~ poly(Household.Head.Age, 3), family = 'poisson', data=data)
poiss_reg4 <- glm(Total.Number.of.Family.members ~ poly(Household.Head.Age, 4), family = 'poisson', data=data)
poiss_reg5 <- glm(Total.Number.of.Family.members ~ poly(Household.Head.Age, 5), family = 'poisson', data=data)
anova(poiss_reg2, poiss_reg3, poiss_reg4, poiss_reg5)

# Test with anova, the result is that polynom with third degree is the best

# poiss_reg5 <- glm(Total.Number.of.Family.members ~ Household.Head.Age, family = 'binomial', data=data) - Error the output of binomial distribution is between 0 and 1 but Total.Number.of.Family.members are integers
# poiss_reg5 <- glm(Total.Number.of.Family.members ~ Household.Head.Age, family = 'quasipoisson', data=data)  - THe result aic is NA

poiss_reg6 <- glm(Total.Number.of.Family.members ~ poly(Household.Head.Age, 2), family = 'gaussian', data=data)
poiss_reg7 <- glm(Total.Number.of.Family.members ~ poly(Household.Head.Age, 3), family = 'gaussian', data=data) # maybe anova test
# Natural spline (df = degrees of freedom)
poiss_reg8 <- glm(Total.Number.of.Family.members ~ ns(Household.Head.Age, df = 3), 
                    family = 'gaussian', data = data)

# GAM (Generalized Additive Model) - smooth spline
poiss_reg9 <- gam(Total.Number.of.Family.members ~ s(Household.Head.Age), 
                 family = gaussian(), data = data)

poiss_reg10 <- glm(Total.Number.of.Family.members ~ ns(Household.Head.Age, df = 3), 
                    family = 'poisson', data = data)

# GAM (Generalized Additive Model) - smooth spline
poiss_reg11 <- gam(Total.Number.of.Family.members ~ s(Household.Head.Age), 
                 family = poisson(), data = data)

poiss_reg12 <- glm(Total.Number.of.Family.members ~ Household.Head.Age, 
                  family = poisson(link = "sqrt"), data = data)

# Poisson with identity link (less common, but worth trying)
poiss_reg13 <- glm(Total.Number.of.Family.members ~ Household.Head.Age, 
                      family = poisson(link = "identity"), data = data)

# Poisson with region (additive)
poiss_reg14 <- glm(Total.Number.of.Family.members ~ poly(Household.Head.Age, 2) + Region, family = 'poisson', data = data)

# Poisson with region (interaction)
poiss_reg15 <- glm(Total.Number.of.Family.members ~ poly(Household.Head.Age, 2) * Region, family = 'poisson', data = data)

# GAM with region
poiss_reg16 <- gam(Total.Number.of.Family.members ~ s(Household.Head.Age) + Region, family = poisson(), data = data)
...
```{r}
result_poly <- data.frame(
  Model = c("Linear GLM", "Poisson (poly 2)", "Poisson (poly 3)", 
            "Poisson (poly 4)", "Poisson (poly 5)"),
  AIC = c(linreg_glm$aic, poiss_reg2$aic, poiss_reg3$aic, 
          poiss_reg4$aic, poiss_reg5$aic)
)
result_poly
```

```{r}
poiss_reg_sp_df3 <- glm(Total.Number.of.Family.members ~ bs(Household.Head.Age, df = 3), family = 'poisson', data = data)
poiss_reg_sp_df4 <- glm(Total.Number.of.Family.members ~ bs(Household.Head.Age, df = 4), family = 'poisson', data = data)
poiss_reg_sp_df5 <- glm(Total.Number.of.Family.members ~ bs(Household.Head.Age, df = 5), family = 'poisson', data = data)
result_splines <- data.frame(
  Model = c("Linear GLM", "Poisson (poly 3)", "Poisson (splines df 3)", 
            "Poisson (splines df 4)", "Poisson (splines df 5)"), 
  AIC = c(linreg_glm$aic, poiss_reg3$aic, poiss_reg_sp_df3$aic, poiss_reg_sp_df4$aic, poiss_reg_sp_df5$aic)
)
result_splines
```

```{r}
# GAM with default smoothing (automatic df selection)
gam_default <- gam(Total.Number.of.Family.members ~ s(Household.Head.Age), 
                   family = poisson(), data = data)

# GAM with specified degrees of freedom (for fair comparison)
gam_df3 <- gam(Total.Number.of.Family.members ~ s(Household.Head.Age, 4), 
               family = poisson(), data = data)

# GAM with more flexibility
gam_df5 <- gam(Total.Number.of.Family.members ~ s(Household.Head.Age, 6), 
               family = poisson(), data = data)

result_gam <- data.frame(
  Model = c("Linear GLM", "Poisson (poly 3)", "Poisson (gam default)", 
            "Poisson (gam df 3)", "Poisson (gam df 5)"), 
  AIC = c(linreg_glm$aic, poiss_reg3$aic, gam_default$aic, gam_df3$aic, gam_df5$aic)
)
result_gam
```

```{r}
poiss_region_feature1 <- glm(Total.Number.of.Family.members ~ Region, family = 'poisson', data = data)
poiss_region_feature2 <- glm(Total.Number.of.Family.members ~ poly(Household.Head.Age, 3) + Region, family = 'poisson', data = data)
result_feature <- data.frame(
  Model = c("Linear GLM", "Poisson (poly 3)", "Poisson (region default)", 
            "Poisson (poly df 3 + region)"), 
  AIC = c(linreg_glm$aic, poiss_reg3$aic, poiss_region_feature1$aic, poiss_region_feature2$aic)
)
result_feature
```
