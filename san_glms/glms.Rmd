...
gam_df4 <- gam(Total.Number.of.Family.members ~ s(Household.Head.Age, 4), 
               family = poisson(), data = data)

# GAM with more flexibility
gam_df6 <- gam(Total.Number.of.Family.members ~ s(Household.Head.Age, 6), 
               family = poisson(), data = data)

result_gam <- data.frame(
  Model = c("Linear GLM", "Poisson (poly 3)", "Poisson (gam default)", 
            "Poisson (gam df 4)", "Poisson (gam df 6)"), 
  AIC = c(linreg_glm$aic, poiss_reg3$aic, gam_default$aic, gam_df4$aic, gam_df6$aic)
)
result_gam
```
```{r}
poiss_reg_poly_sqrt <- glm(Total.Number.of.Family.members ~ poly(Household.Head.Age, 3), family = 'poisson'(link = "sqrt"), data=data)
poiss_reg_poly_iden <- glm(Total.Number.of.Family.members ~ Household.Head.Age, 
                      family = poisson(link = "identity"), data = data)

result_links <- data.frame(
  Model = c("Poisson (poly 3)", "Poisson (poly 3 + square root)", 
            "Poisson (poly 3 + identity)"), 
  AIC = c(poiss_reg3$aic, poiss_reg_poly_sqrt$aic, poiss_reg_poly_iden$aic)
)
result_links
```

```{r}
poiss_poly_region <- glm(Total.Number.of.Family.members ~ poly(Household.Head.Age, 3) + Region, family = 'poisson', data = data)
poiss_poly_region$aic
```
...
I compared several model specifications using ANOVA tests and AIC. For the age variable, both criteria showed that a degree-3 polynomial provides the best trade-off between fit and complexity. Spline models and GAMs yielded equal or higher AIC, so the cubic polynomial captures the non-linearity sufficiently well. Alternative link functions were also tested but all performed worse than the canonical log link. After selecting the age structure, I added the categorical variable region, encoded with dummy variables, which improved the model.

The final model - a Poisson GLM with log link, age as a degree-3 polynomial, and region as a categorical predictor - achieved the lowest AIC (68252.06). This model is appropriate because Poisson GLMs naturally model count data, the polynomial captures the nonlinear age effect without overfitting, and including region adds relevant explanatory power. Therefore, this is the recommended model.